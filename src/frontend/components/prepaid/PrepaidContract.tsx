...
interface IPrepaidContractState {
	activeCardIndex: number,
	sum: number,
}

interface IPrepaiContractProps {
	cards: any,
	activeCard: any,
	onSubmit: any,
}

class PrepaidContract extends React.Component<IPrepaiContractProps, IPrepaidContractState> {
	...
	onSubmitForm(event: any) {
		...
		const {activeCard, onSubmit, cards}: any = this.props;
		const {activeCardIndex}: any = this.state;
		const isNumber = !isNaN(parseFloat(sum)) && isFinite(sum);
		if (!isNumber || sum <= 0) {
			return;
		}
		onSubmit(cards[activeCardIndex].id, Number(sum));
	}

	render() {
		const {cards, activeCard}: any = this.props;
		const {activeCardIndex}: any = this.state;
